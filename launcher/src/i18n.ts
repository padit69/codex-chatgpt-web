...
const en = {
  ...
  apiGatewaySection: "Gateway",
  apiGatewayBody: "The Codex Responses route stays unauthenticated on its own port. The gateway is a separate listener that accepts only requests carrying a token created here, so it is the only surface safe to put a tunnel in front of.",
  apiGatewayOn: "Enabled",
  apiGatewayOff: "Disabled",
  apiPortLabel: "Port",
  apiEndpointLabel: "Endpoint",
  apiEnable: "Enable gateway",
  apiDisable: "Disable gateway",
  apiRestartNotice: "Enabling or disabling the gateway restarts the local daemon. Finish active turns first.",
  apiTunnelNotice: "The gateway binds to 127.0.0.1 only. Bring your own tunnel or reverse proxy to publish it, and keep every token secret: whoever holds one can use your ChatGPT account.",
  apiTokensSection: "API tokens",
  apiTokensBody: "Give each client its own token. Only a hash is stored, so a token is shown once at creation and can be revoked individually.",
  apiTokenNameLabel: "Token name",
  apiTokenNamePlaceholder: "laptop, n8n, CI…",
  apiCreateToken: "Create token",
  apiTokenCreatedTitle: "Token created",
  apiTokenCreatedBody: "Copy it now. It is shown once and cannot be recovered.",
  apiCopyToken: "Copy",
  apiTokenCopied: "Copied",
  apiRevokeToken: "Revoke",
  apiNoTokens: "No tokens yet. Create one to enable the gateway.",
  apiTokenCreatedAt: "Created",
  apiTokenLastUsed: "Last used",
  apiTokenNeverUsed: "never",
  apiTokenRequired: "Create at least one API token before enabling the gateway.",
  apiUsageSection: "Using it",
  apiUsageBody: "The gateway speaks the OpenAI Responses API. Call POST /v1/responses with an Authorization: Bearer header and a chatgpt-web/… model id; GET /v1/models lists the ids this account can use. Set stream: true for incremental Server-Sent Events.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  apiGatewaySection: "网关",
  apiGatewayBody: "Codex Responses 路由在其自身端口上仍然不带鉴权。网关是独立的监听器，只接受携带此处所创建令牌的请求，因此它是唯一适合放在隧道后面的入口。",
  apiGatewayOn: "已启用",
  apiGatewayOff: "已停用",
  apiPortLabel: "端口",
  apiEndpointLabel: "访问地址",
  apiEnable: "启用网关",
  apiDisable: "停用网关",
  apiRestartNotice: "启用或停用网关会重启本地守护进程。请先结束进行中的任务。",
  apiTunnelNotice: "网关只绑定 127.0.0.1。请自行使用隧道或反向代理进行发布，并妥善保管每个令牌：持有者可以使用你的 ChatGPT 账户。",
  apiTokensSection: "API 令牌",
  apiTokensBody: "为每个客户端分配独立令牌。系统只保存哈希值，因此令牌仅在创建时显示一次，并且可以单独吊销。",
  apiTokenNameLabel: "令牌名称",
  apiTokenNamePlaceholder: "笔记本、n8n、CI…",
  apiCreateToken: "创建令牌",
  apiTokenCreatedTitle: "令牌已创建",
  apiTokenCreatedBody: "请立即复制。它只显示一次且无法找回。",
  apiCopyToken: "复制",
  apiTokenCopied: "已复制",
  apiRevokeToken: "吊销",
  apiNoTokens: "尚无令牌。创建一个即可启用网关。",
  apiTokenCreatedAt: "创建于",
  apiTokenLastUsed: "最近使用",
  apiTokenNeverUsed: "从未使用",
  apiTokenRequired: "启用网关前请至少创建一个 API 令牌。",
  apiUsageSection: "使用方式",
  apiUsageBody: "网关使用 OpenAI Responses API。请带上 Authorization: Bearer 请求头调用 POST /v1/responses，并指定 chatgpt-web/… 模型 ID；GET /v1/models 会列出该账户可用的 ID。设置 stream: true 可获得增量 SSE 流。",
  product: "Codex Web GPT",
  devBadge: "DEV",
...
const ja: Record<keyof typeof en, string> = {
  ...
  apiGatewaySection: "ゲートウェイ",
  apiGatewayBody: "Codex の Responses ルートは専用ポート上で認証なしのままです。ゲートウェイは独立したリスナーで、ここで作成したトークンを持つリクエストだけを受け付けるため、トンネルの背後に置ける唯一の入口です。",
  apiGatewayOn: "有効",
  apiGatewayOff: "無効",
  apiPortLabel: "ポート",
  apiEndpointLabel: "エンドポイント",
  apiEnable: "ゲートウェイを有効化",
  apiDisable: "ゲートウェイを無効化",
  apiRestartNotice: "ゲートウェイの有効化と無効化はローカルデーモンを再起動します。実行中のターンを先に終了してください。",
  apiTunnelNotice: "ゲートウェイは 127.0.0.1 のみにバインドします。公開するにはご自身のトンネルまたはリバースプロキシを用意し、各トークンは秘密にしてください。保持者はあなたの ChatGPT アカウントを利用できます。",
  apiTokensSection: "API トークン",
  apiTokensBody: "クライアントごとに個別のトークンを発行します。保存されるのはハッシュのみで、トークンは作成時に一度だけ表示され、個別に失効できます。",
  apiTokenNameLabel: "トークン名",
  apiTokenNamePlaceholder: "ノート PC、n8n、CI…",
  apiCreateToken: "トークンを作成",
  apiTokenCreatedTitle: "トークンを作成しました",
  apiTokenCreatedBody: "今すぐコピーしてください。一度しか表示されず、復元できません。",
  apiCopyToken: "コピー",
  apiTokenCopied: "コピーしました",
  apiRevokeToken: "失効",
  apiNoTokens: "トークンがありません。作成するとゲートウェイを有効化できます。",
  apiTokenCreatedAt: "作成日時",
  apiTokenLastUsed: "最終使用",
  apiTokenNeverUsed: "未使用",
  apiTokenRequired: "ゲートウェイを有効化する前に、API トークンを 1 つ以上作成してください。",
  apiUsageSection: "使い方",
  apiUsageBody: "ゲートウェイは OpenAI Responses API を話します。Authorization: Bearer ヘッダーを付けて POST /v1/responses を呼び出し、chatgpt-web/… のモデル ID を指定します。GET /v1/models でアカウントが利用できる ID を一覧できます。stream: true を設定すると増分の SSE が得られます。",
  product: "Codex Web GPT",
  devBadge: "DEV",
